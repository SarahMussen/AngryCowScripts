...
public class FarmerController : NetworkBehaviour
{
    ...
    public override void OnStartLocalPlayer() 
    {
        ...
        playerCamera.transform.localPosition = new Vector3(0, 7, -8);
        float y = Input.GetAxis("Vertical");
        playerCamera.transform.localRotation = Quaternion.Euler(10, -y, 0);

        var kinectmanager = playerCamera.gameObject.GetComponent<KinectManager>();
        kinectmanager.enabled = true;
        kinectmanager.StartManager();

    }

    public void SetKinectManager()
    {
        Camera playerCamera = Camera.main;
        playerCamera.gameObject.AddComponent<KinectManager>();
        playerCamera.gameObject.GetComponent<KinectManager>().enabled = true;
    }
    ...
    void Start()
    {
        GameManager.Instance.setBodyOfFarmer(this.gameObject);
